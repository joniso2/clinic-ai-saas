...
export async function PATCH(
  ...
  let body: { service_name?: string; price?: number; duration_minutes?: number; aliases?: string[]; is_active?: boolean; description?: string | null; category?: string | null; color?: string | null };
  try {
    body = await req.json();
  } catch {
  ...
  const { data, error } = await supabase
    ...
    .eq('id', id)
    .eq('clinic_id', row.clinic_id)
    .select('id, service_name, price, duration_minutes, aliases, is_active, category, color')
    .single();
