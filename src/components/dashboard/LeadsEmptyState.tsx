...
export function LeadsEmptyState({ onAddLead }: { onAddLead: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-gradient-to-b from-slate-50/80 to-white px-6 py-16 text-center">
      ...
      <p className="mt-2 max-w-sm text-sm text-slate-500 dark:text-slate-400 text-center">
        הוסף את הליד הראשון כדי להתחיל. פניות חדשות יופיעו כאן אוטומטית.
      </p>
      <button
        type="button"
        onClick={onAddLead}
        className="mt-6 rounded-xl bg-slate-900 dark:bg-slate-100 px-5 py-2.5 text-sm font-semibold text-white dark:text-slate-900 shadow-sm transition hover:bg-slate-800 dark:hover:bg-white focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2"
      >
        הוסף ליד ראשון
      </button>
    </div>
