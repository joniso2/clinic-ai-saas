...
export function PhoneContactModal({ phone, onClose }: { phone: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-xs rounded-2xl bg-white dark:bg-slate-900 shadow-2xl overflow-hidden modal-enter" dir="rtl" onClick={(e) => e.stopPropagation()}>
        <div className="bg-slate-900 px-6 py-4">
          <div className="flex items-center justify-between gap-3">
            <button type="button" onClick={onClose} className="rounded-full p-1.5 text-slate-400 hover:bg-white/10 hover:text-white transition-colors shrink-0" aria-label="סגור">
              <X className="h-5 w-5" />
            </button>
            <h2 className="text-lg font-bold text-white text-center flex-1">יצירת קשר</h2>
            <div className="w-9" />
          </div>
        </div>
...
export function PendingReviewModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 px-4 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-md max-h-[90dvh] rounded-2xl bg-white dark:bg-slate-900 shadow-2xl overflow-hidden modal-enter flex flex-col" dir="rtl" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
...
export function CompleteLeadModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 px-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="סיום טיפול" onClick={onClose}>
      <div className="w-full max-w-sm max-h-[90dvh] rounded-2xl bg-white dark:bg-slate-900 shadow-2xl overflow-hidden modal-enter flex flex-col" dir="rtl" onClick={(e) => e.stopPropagation()}>
        <div className="bg-slate-900 px-6 py-4 shrink-0">
          <div className="flex items-center justify-between gap-3">
            <button type="button" onClick={onClose} className="rounded-full p-1.5 text-slate-400 hover:bg-white/10 hover:text-white transition-colors shrink-0" aria-label="סגור">
              <X className="h-5 w-5" />
            </button>
            <h2 className="text-lg font-bold text-white text-center flex-1">סיום טיפול</h2>
            <div className="w-9" />
          </div>
          <p className="text-xs text-slate-400 text-center mt-1">{lead.full_name || 'ליד ללא שם'}</p>
        </div>
        ...
        <div className="shrink-0 flex items-center justify-between gap-3 border-t border-slate-100 dark:border-slate-800 px-6 py-4 bg-slate-50/50 dark:bg-slate-800/30">
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-2.5 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 hover:border-slate-300 transition-colors"
          >
            ביטול
          </button>
          <button
