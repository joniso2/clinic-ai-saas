...
type AudienceType = 'all' | 'custom' | 'last_visit_filter';
type ScheduleType = 'now' | 'weekly' | 'monthly' | 'auto_days_after';
...
const VARIABLES = [
  ...
  { label: '{שם מרפאה}',  value: '{שם מרפאה}',  title: 'שם המרפאה' },
];
...
function getStatusBadge(status: Campaign['status']) {
  switch (status) {
    ...
    default:          return { label: 'טיוטה',   cls: 'bg-slate-100 text-slate-500 dark:bg-zinc-800 dark:text-zinc-400' };
  }
}

function getChannelBadge(channel: Campaign['channel']) {
  ...
  return (
    <div className="space-y-5 p-5">
      ...
      <section>
        ...
        <div className="inline-flex rounded-xl border border-slate-200 dark:border-zinc-700 overflow-hidden bg-slate-50 dark:bg-zinc-800/50 p-1 gap-1">
      ...
      <section>
        ...
        <div className="rounded-xl border border-slate-200 dark:border-zinc-700 overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500/30 focus-within:border-indigo-400 transition">
          <textarea
            ref={textareaRef}
            value={message}
            ...
            rows={5}
            dir="rtl"
            className="w-full bg-white dark:bg-zinc-800/60 px-4 py-3 text-sm text-slate-900 dark:text-zinc-100 placeholder:text-slate-400 dark:placeholder:text-zinc-500 focus:outline-none resize-none text-right"
          />
          <div className="flex items-center justify-between border-t border-slate-100 dark:border-zinc-800 px-4 py-2 bg-slate-50/60 dark:bg-zinc-800/40">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[11px] font-medium text-slate-400 dark:text-zinc-500 ml-1">הוסף משתנה:</span>
              {VARIABLES.map((v) => (
                <button
                  key={v.value}
                  type="button"
            ...
            <span className={`text-[11px] tabular-nums ${message.length > 140 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400 dark:text-zinc-500'}`}>
              {message.length} / 160
            </span>
          </div>
        </div>
          ...
          {[
            ...
            <label
              ...
              className={`flex items-center gap-3 rounded-xl border p-3.5 cursor-pointer transition ${
                audienceType === opt.value
                  ...
                  : 'border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/40 hover:border-slate-300 dark:hover:border-zinc-600'
            ...
            >
              ...
              {opt.value === 'last_visit_filter' ? (
                <span className="flex items-center gap-2 flex-1 text-sm text-slate-700 dark:text-zinc-300">
                  ...
                  <input
                    ...
                    className="w-14 rounded-lg border border-slate-200 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-2 py-1 text-sm text-center tabular-nums focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
                  />
                  <span>ימים אחרונים</span>
                  <span className="mr-auto rounded-full bg-slate-100 dark:bg-zinc-700/80 px-2 py-0.5 text-xs font-medium text-slate-500 dark:text-zinc-400 tabular-nums">
                ...
                <span className="flex items-center justify-between flex-1">
                  ...
                  {opt.count != null && (
                    <span className="rounded-full bg-slate-100 dark:bg-zinc-700/80 px-2 py-0.5 text-xs font-medium text-slate-500 dark:text-zinc-400 tabular-nums">
                      {opt.count} לקוחות
                    </span>
                  )}
                </span>
      ...
      <section>
        ...
        <div className="space-y-2">
          <label className={`flex items-center gap-3 rounded-xl border p-3.5 cursor-pointer transition ${scheduleType === 'now' ? 'border-indigo-300 dark:border-indigo-700 bg-indigo-50/60 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/40 hover:border-slate-300 dark:hover:border-zinc-600'}`}>
            <input type="radio" name="schedule" value="now" checked={scheduleType === 'now'} onChange={() => setScheduleType('now')} className="text-indigo-500" />
            <Send className="h-4 w-4 text-slate-400 dark:text-zinc-500 shrink-0" />
            <span className="text-sm text-slate-700 dark:text-zinc-300">שלח עכשיו</span>
          </label>

          <label className={`flex items-center gap-3 rounded-xl border p-3.5 cursor-pointer transition ${scheduleType === 'weekly' ? 'border-indigo-300 dark:border-indigo-700 bg-indigo-50/60 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/40 hover:border-slate-300 dark:hover:border-zinc-600'}`}>
            ...
            <Repeat className="h-4 w-4 text-slate-400 dark:text-zinc-500 shrink-0" />
            <span className="flex items-center gap-2 flex-1 text-sm text-slate-700 dark:text-zinc-300">
              <span>שבועי — יום</span>
              <div className="relative">
                <select
                  ...
                  className="appearance-none rounded-lg border border-slate-200 dark:border-zinc-600 bg-white dark:bg-zinc-800 pr-7 pl-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
                  dir="rtl"
                >
          ...
          <label className={`flex items-center gap-3 rounded-xl border p-3.5 cursor-pointer transition ${scheduleType === 'monthly' ? 'border-indigo-300 dark:border-indigo-700 bg-indigo-50/60 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/40 hover:border-slate-300 dark:hover:border-zinc-600'}`}>
          ...
          <label className={`flex items-center gap-3 rounded-xl border p-3.5 cursor-pointer transition ${scheduleType === 'auto_days_after' ? 'border-indigo-300 dark:border-indigo-700 bg-indigo-50/60 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/40 hover:border-slate-300 dark:hover:border-zinc-600'}`}>
      ...
      {/* Audience preview */}
      {audienceSize > 0 && (
        <div className="rounded-xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-200/80 dark:border-zinc-700/60 p-3.5">
          <p className="text-xs font-medium text-slate-500 dark:text-zinc-400 mb-2">תצוגה מקדימה — {audienceSize} נמענים</p>
          <div className="flex flex-wrap gap-1.5 max-h-20 overflow-y-auto">
            {audienceCustomers.slice(0, 12).map((c) => (
              <span key={c.id} className="inline-flex items-center gap-1.5 rounded-full bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 px-2 py-0.5 text-xs text-slate-700 dark:text-zinc-300">
                {c.full_name.split(' ')[0]}
              </span>
            ))}
            {audienceSize > 12 && (
              <span className="rounded-full bg-slate-100 dark:bg-zinc-700 px-2 py-0.5 text-xs text-slate-500 dark:text-zinc-400">+{audienceSize - 12}</span>
            )}
          </div>
        </div>
      )}
    ...
    <div className="p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          type="button"
          onClick={onRefresh}
          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-700 transition"
        >
      ...
      {campaigns.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 dark:bg-zinc-800 mb-4">
            <Send className="h-7 w-7 text-slate-400 dark:text-zinc-500" />
          </div>
          <p className="text-sm font-medium text-slate-600 dark:text-zinc-300">אין קמפיינים עדיין</p>
          <p className="text-xs text-slate-400 dark:text-zinc-500 mt-1">הקמפיינים שתשלח יופיעו כאן</p>
        </div>
      ) : (
        <div className="rounded-xl border border-slate-200 dark:border-zinc-700 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-right text-sm" dir="rtl">
              <thead className="bg-slate-50 dark:bg-zinc-800/80 text-[11px] font-semibold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">
              ...
              <tbody>
                {campaigns.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="border-b border-slate-50 dark:border-zinc-800/60 last:border-0 hover:bg-slate-50/60 dark:hover:bg-zinc-800/30 transition-colors">
                      <td className="py-3 px-4 text-slate-500 dark:text-zinc-400 whitespace-nowrap">{formatDate(c.created_at)}</td>
                      ...
                      <td className="py-3 px-4 text-slate-600 dark:text-zinc-300 tabular-nums">{c.audience_size}</td>
                      <td className="py-3 px-4 text-slate-700 dark:text-zinc-200 max-w-[200px]">
                        <span className="truncate block" title={c.message}>{c.message}</span>
                      </td>
                      <td className="py-3 px-4 text-slate-500 dark:text-zinc-400 whitespace-nowrap text-xs">{scheduleLabel}</td>
                      <td className="py-3 px-4">
...
export function MessagingPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-8 px-4 pb-4" dir="rtl">
      ...
      <div
        className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-700 shadow-2xl overflow-hidden flex flex-col"
        style={{ maxHeight: 'calc(100vh - 4rem)', animation: 'slideUpFade 200ms ease-out forwards' }}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800 px-6 py-4 bg-white dark:bg-zinc-900 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-slate-900 dark:text-zinc-100">מערכת הודעות</h2>
              <p className="text-xs text-slate-400 dark:text-zinc-500">{customers.length} לקוחות זמינים</p>
            </div>
          </div>
        ...
        {/* Tabs */}
        <div className="flex border-b border-slate-100 dark:border-zinc-800 px-6 bg-slate-50/60 dark:bg-zinc-800/40 shrink-0">
          {[
            ...
            >
              ...
              {t.id === 'log' && campaigns.length > 0 && (
                <span className="rounded-full bg-slate-200 dark:bg-zinc-700 px-1.5 py-0.5 text-[10px] font-semibold text-slate-600 dark:text-zinc-300 tabular-nums">
                  {campaigns.length}
                </span>
              )}
            </button>
