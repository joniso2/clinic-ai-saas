...
const ROLES = [
  ...
  { value: 'member', label: 'Member', icon: User,   color: 'text-slate-600 dark:text-zinc-400 bg-slate-100 dark:bg-zinc-700 border-slate-200 dark:border-zinc-600' },
];
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 overflow-hidden">
        <div className="border-b border-slate-100 dark:border-zinc-700 bg-slate-50/60 dark:bg-zinc-700/60 px-5 py-4 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-zinc-100">Team members</h3>
            <p className="text-xs text-slate-500 dark:text-zinc-400">Users with access to this clinic.</p>
          </div>
        </div>
        <div className="divide-y divide-slate-100 dark:divide-zinc-700">
          {loading ? (
            <div className="flex items-center gap-2 px-5 py-4 text-sm text-slate-400 dark:text-zinc-500">
              <Loader2 className="h-4 w-4 animate-spin" />
              Loading members…
            </div>
          ) : members.length === 0 ? (
            <div className="px-5 py-6 text-center">
              <p className="text-sm text-slate-400 dark:text-zinc-500">No team members found.</p>
            </div>
          ) : (
            members.map((member) => (
              <div key={member.user_id} className="flex items-center gap-4 px-5 py-3.5">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 dark:bg-zinc-700 text-sm font-semibold text-slate-600 dark:text-zinc-300">
                  {member.email.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-900 dark:text-zinc-100 truncate">{member.email}</p>
                </div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 overflow-hidden">
        <div className="border-b border-slate-100 dark:border-zinc-700 bg-slate-50/60 dark:bg-zinc-700/60 px-5 py-4 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-zinc-100">Invite member</h3>
            <p className="text-xs text-slate-500 dark:text-zinc-400">Send an invitation email to add someone to your team.</p>
          </div>
        </div>
        <div className="px-5 py-5 space-y-4">
          <div className="flex flex-wrap gap-3">
            <div className="flex-1 min-w-48 relative">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 dark:text-zinc-500 pointer-events-none" />
              <input
                type="email"
                value={inviteEmail}
                onChange={(e) => { setInviteEmail(e.target.value); setInviteStatus('idle'); }}
                placeholder="colleague@example.com"
                className="w-full rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-700/50 pl-9 pr-3.5 py-2.5 text-sm text-slate-900 dark:text-zinc-100 placeholder:text-slate-400 dark:placeholder:text-zinc-500 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900/40 transition-colors"
              />
            </div>
            <select
              value={inviteRole}
              onChange={(e) => setInviteRole(e.target.value)}
              className="rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-700/50 px-3.5 py-2.5 text-sm text-slate-900 dark:text-zinc-100 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900/40 transition-colors"
            >
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-zinc-700">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-zinc-100">Role permissions</h3>
        </div>
        <div className="divide-y divide-slate-100 dark:divide-zinc-700">
          {[
