  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="p-4 border-b border-slate-700 flex items-center justify-between flex-row-reverse">
          <h3 className="text-lg font-semibold text-slate-100 flex items-center gap-2 flex-row-reverse"><Radio className="h-5 w-5" /> עדכון אחרון</h3>
          <button type="button" onClick={fetchLogs} disabled={loading} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400 disabled:opacity-50 flex items-center gap-1.5">
          ...
          <table className="w-full text-sm text-right">
            ...
            <tbody>
              {loading && logs.length === 0 ? (
                <tr><td colSpan={7} className="py-12 text-center text-slate-500">טוען…</td></tr>
              ) : logs.length === 0 ? (
                <tr><td colSpan={7} className="py-12 text-center text-slate-500">אין הודעות לאחרונה</td></tr>
              ) : (
                logs.map((row) => (
                  <tr key={row.id} className="border-b border-slate-700 hover:bg-slate-800/50">
                    <td className="py-2 px-4 text-xs text-slate-400 whitespace-nowrap">{new Date(row.created_at).toLocaleString('he-IL')}</td>
                    ...
                    <td className="py-2 px-4 text-slate-400 max-w-[220px] truncate">{row.message_preview}</td>
