    ...
    <>
      <div className="mb-6 text-right">
        <Link
          ...
          className="inline-flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-50 mb-2"
        >
          <ArrowRight className="h-4 w-4" />
          חזרה לקליניקות
        </Link>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">לקוחות · {clinicName}</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">צפייה ועריכה לפי קליניקה</p>
      </div>

      {!loading && customers.length > 0 && (
        <div className="mb-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-4 py-3">
          <span className="text-sm font-medium text-slate-500 dark:text-slate-400">סה״כ הכנסות (לקוחות): </span>
          <span className="text-lg font-semibold text-slate-900 dark:text-slate-50">{formatCurrencyILS(revenueSummary)}</span>
        </div>
      )}
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center flex-row-reverse">
        ...
        <div ref={filterRef} className="relative">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm"
          >
            מסננים
          ...
          {filterOpen && (
            <div className="absolute top-full left-0 mt-1 z-50 w-48 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 shadow-lg p-3 text-right" dir="rtl">
              ...
              <select
                ...
                className="w-full rounded border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-2 py-1.5 text-sm mt-1"
              >
      ...
      {loading && customers.length === 0 ? (
        ...
        <div className="rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-950/50 px-6 py-12 text-center" dir="rtl">
          <p className="text-sm text-slate-500 dark:text-slate-400">אין לקוחות לקליניקה זו.</p>
        </div>
      ) : (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-right" dir="rtl">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">
              ...
              <tbody>
                {customers.map((c) => (
                  <tr key={c.id} className="border-b border-slate-100 dark:border-slate-800/80 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 py-2">
                    <td className="py-2 px-3 font-medium text-slate-900 dark:text-slate-50">{c.full_name}</td>
                    <td className="py-2 px-3 text-slate-600 dark:text-slate-400" dir="ltr">{c.phone}</td>
                    <td className="py-2 px-3 text-slate-600 dark:text-slate-400">{formatDate(c.last_visit_date)}</td>
                    <td className="py-2 px-3 text-slate-600 dark:text-slate-400">{c.visits_count}</td>
                    <td className="py-2 px-3 font-medium text-slate-800 dark:text-slate-200">{formatCurrencyILS(Number(c.total_revenue))}</td>
                    ...
                    <td className="py-2 px-3">
                      <div className="flex items-center gap-1 justify-start">
                        <button type="button" onClick={() => setDetailId(c.id)} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700" aria-label="צפה">
              ...
              {detailLoading && !detailCustomer ? (
                ...
                <>
                  <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-4 space-y-3">
                    <p className="font-semibold text-slate-900 dark:text-slate-50 text-lg">{detailCustomer.full_name}</p>
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">תוריים שהסתיימו</h3>
                    {detailAppointments.length === 0 ? (
                      <p className="text-sm text-slate-500 dark:text-slate-400">אין תוריים להצגה</p>
                    ) : (
                      <ul className="space-y-2">
                        {detailAppointments.map((apt) => (
                          <li key={apt.id} className="rounded-lg border border-slate-100 dark:border-slate-800 px-3 py-2 text-sm">
                            ...
                            {apt.notes && <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{apt.notes}</p>}
                          </li>
                        ))}
                      </ul>
                    )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 p-5 shadow-xl text-right max-w-sm w-full" dir="rtl">
            ...
            <div className="mt-4 flex gap-2 justify-start">
              <button type="button" onClick={() => setDeleteId(null)} className="rounded-xl border border-slate-200 dark:border-slate-600 px-4 py-2 text-sm font-medium">ביטול</button>
