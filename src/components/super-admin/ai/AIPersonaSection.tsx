...
function RadioCard<T extends string>({
  ...
  return (
    ...
    >
      <div className={`rounded-lg p-2 ${selected ? 'bg-indigo-500/20 text-indigo-300' : 'bg-slate-800 text-slate-400'} transition-colors`}>
        <Icon className="h-4 w-4" />
      </div>
      <div className="flex-1 min-w-0 text-right">
        <p className={`text-sm font-semibold ${selected ? 'text-slate-100' : 'text-slate-300'}`}>{title}</p>
  ...
  return (
    <div>
      ...
      <select
        ...
        className={`${input} text-right appearance-none`}
      >
        {options.map((o) => (
    ...
    <div dir="rtl" className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-row-reverse gap-4 flex-wrap">
        <div className="text-right">
          <h2 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">פרסונה והתנהגות AI</h2>
      ...
      {clinics.length > 0 && (
        <div className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
          <div className="flex items-center gap-4 flex-row-reverse flex-wrap">
            <label className="text-xs font-medium text-slate-400 shrink-0">קליניקה</label>
            <select
              value={selectedClinicId}
              onChange={(e) => handleClinicChange(e.target.value)}
              className="flex-1 min-w-[200px] rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500 text-right"
            >
              {clinics.map((c) => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6 items-start">
        ...
        <div className="xl:col-span-3 space-y-5">
          ...
          <section className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
            <div className="mb-4 text-right">
              <h3 className="text-sm font-semibold text-slate-100">סוג עסק / תעשייה</h3>
              <p className="text-xs text-slate-500 mt-0.5">בחר את ההקשר שבו פועלת הקליניקה — ה-AI יתאים את ההתנהגות בהתאם.</p>
            </div>
          ...
          <section className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
            <div className="mb-4 text-right">
              <h3 className="text-sm font-semibold text-slate-100">אסטרטגיית שיחה</h3>
              <p className="text-xs text-slate-500 mt-0.5">כיצד ה-AI מוביל את השיחה לפעולה הבאה.</p>
            </div>
            ...
            <div className="mt-3 rounded-lg bg-slate-800/60 px-4 py-2.5 text-xs text-slate-400 text-right leading-relaxed">
          ...
          {/* Tone & length */}
          <section className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
            <h3 className="text-sm font-semibold text-slate-100 mb-4 text-right">טון ואורך תגובה</h3>
            ...
            {/* Strict hours toggle */}
            <div className="flex items-center justify-between flex-row-reverse mt-4 pt-4 border-t border-slate-800">
              <div className="text-right">
                <span className="text-sm text-slate-300 font-medium">אכיפת שעות עבודה קפדנית</span>
                <p className="text-xs text-slate-500 mt-0.5">כבוי = ה-AI מפנה ללא סירוב כשפונים מחוץ לשעות</p>
              </div>
              <button
                type="button"
                ...
                className={`relative inline-flex h-5 w-9 rounded-full transition-colors duration-200 ${
                  form.strict_hours_enforcement ? 'bg-indigo-600' : 'bg-slate-700'
                }`}
              >
                <span
                  className={`absolute top-0.5 h-4 w-4 rounded-full bg-slate-200 shadow transition-all duration-200 ${
          ...
          <section className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
            <h3 className="text-sm font-semibold text-slate-100 mb-1 text-right">תיאור העסק</h3>
            <p className="text-xs text-slate-500 mb-3 text-right">תיאור קצר של הקליניקה, מומחיות ייחודית, ושירותים מרכזיים — ה-AI ישתמש בזה כהקשר.</p>
            <textarea
              ...
              className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500 resize-none text-right leading-relaxed"
            />
            <p className="text-[11px] text-slate-600 mt-1 text-right">{form.business_description.length} תווים</p>
          </section>

          {/* Custom prompt override */}
          <section className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
            <div className="flex items-start justify-between flex-row-reverse mb-1 gap-2">
              <h3 className="text-sm font-semibold text-slate-100 text-right">הוראות מותאמות אישית</h3>
              <span className="shrink-0 rounded-md bg-indigo-500/10 text-indigo-300 text-[10px] font-semibold px-2 py-0.5 border border-indigo-500/20">מתקדם</span>
            </div>
            <p className="text-xs text-slate-500 mb-3 text-right">הוראות נוספות שיתווספו בסוף ה-system prompt. משמשות לחריגות ספציפיות, מוצרים חדשים, או הנחיות עונתיות.</p>
            <textarea
              ...
              className="w-full rounded-xl border border-slate-700 bg-slate-900 px-4 py-3 text-sm text-slate-300 placeholder:text-slate-700 focus:outline-none focus:border-indigo-500 resize-none font-mono leading-relaxed"
            />
            <p className="text-[11px] text-slate-600 mt-1 text-right">{form.custom_prompt_override.length} תווים</p>
          </section>
        ...
        <div className="xl:col-span-2">
          <div className="sticky top-6 rounded-xl overflow-hidden border border-slate-800 shadow-2xl">
            {/* Preview header */}
            <div className="flex items-center justify-between flex-row-reverse px-4 py-3 bg-slate-900 border-b border-slate-800">
              <div className="flex items-center gap-2 flex-row-reverse">
                <Eye className="h-4 w-4 text-slate-500" />
                <span className="text-xs font-semibold text-slate-300">תצוגה מקדימה</span>
              </div>
            ...
            {/* Preview body */}
            <div className="bg-slate-950 overflow-y-auto max-h-[calc(100dvh-12rem)] p-4 space-y-3">
              {previewSegments.map((seg) => (
                ...
                >
                  ...
                  <pre
                    className="text-[11px] text-slate-400 whitespace-pre-wrap break-words font-mono leading-relaxed"
              ...
              <div className="rounded-lg border border-slate-800 p-3 bg-slate-900/60">
                <p className="text-[10px] font-mono font-bold mb-1 text-slate-600">// TOTAL SEGMENTS</p>
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 start-1/2 -translate-x-1/2 z-[70] rounded-xl bg-slate-800 border border-slate-700 text-slate-100 px-5 py-2.5 text-sm font-medium shadow-xl whitespace-nowrap">
          {toast}
        </div>
      )}
    </div>
