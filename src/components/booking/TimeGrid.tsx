  ...
  if (slots.length === 0) {
    return (
      <div className="px-4 py-16 text-center">
        <p className="text-gray-500 text-lg font-medium">אין שעות פנויות</p>
        <p className="text-gray-400 text-sm mt-1">בחר תאריך אחר</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 pt-5 pb-4">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500 mt-0.5">{formatDateHebrew(date)}</p>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full py-3.5 rounded-full text-base font-semibold transition-all duration-150 active:scale-[0.98] touch-manipulation shadow-[0_2px_8px_rgba(0,0,0,0.06)]
                ${isSelected ? 'bg-black text-white shadow-md' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}
              `}
            >
              {time}
            </button>
