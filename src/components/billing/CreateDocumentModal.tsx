...
const INPUT = `w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800
  px-3 py-2 text-sm text-slate-900 dark:text-slate-50 placeholder:text-slate-400 dark:placeholder:text-slate-500
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div ref={panelRef} className="modal-enter w-full max-w-2xl bg-white dark:bg-slate-950 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]" dir="rtl" role="dialog" aria-modal="true" aria-label="יצירת מסמך">
        ...
        {/* Step indicators */}
        <div className="flex border-b border-slate-100 dark:border-slate-800 px-6 py-3 shrink-0 gap-0">
          {STEPS.map((label, i) => (
            <div key={i} className="flex items-center">
              <div className={`flex items-center gap-1.5 text-xs font-medium transition-colors
                ${i === step ? 'text-slate-900 dark:text-slate-50' : i < step ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500'}`}>
                <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full text-[10px]
                  ${i === step ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900'
              ...
              {i < STEPS.length - 1 && (
                <div className={`mx-2 h-px w-6 ${i < step ? 'bg-emerald-300 dark:bg-emerald-700' : 'bg-slate-200 dark:bg-slate-700'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 0 && (
            <div className="space-y-5">
              ...
              {!appointmentId && (
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">מקור</label>
          ...
          {step === 1 && (
            <div className="space-y-5">
                    ...
                    {services.map((svc) => (
                      <button key={svc.id} onClick={() => addServiceAsItem(svc)}
                        className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-700
                          px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                        <Plus className="h-3 w-3" />
                        {svc.service_name}
                        <span className="text-slate-400 dark:text-slate-500">₪{svc.price}</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {loadingServices && (
                <p className="text-xs text-slate-400 dark:text-slate-500">טוען שירותים...</p>
              )}
                    ...
                    <div key={idx} className="grid grid-cols-[1fr_56px_96px_32px] gap-2 items-center">
                      ...
                      <button onClick={() => removeItem(idx)} disabled={items.length === 1}
                        className="flex items-center justify-center h-9 w-8 rounded-lg text-slate-400
                          hover:text-red-500 dark:text-slate-500 dark:hover:text-red-400 transition-colors
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div className="bg-slate-50 dark:bg-slate-800 px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 mb-1">
                    {DOC_TYPE_LABELS[docType]}
                  </p>
                  <p className="font-semibold text-slate-900 dark:text-slate-50">{settings.business_name}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">מס׳ עסק: {settings.business_number}</p>
                </div>
                <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">לקוח</p>
                  <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{customerName}</p>
                  {customerPhone && <p className="text-xs text-slate-500 dark:text-slate-400">{customerPhone}</p>}
                </div>
                <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                  <table className="w-full text-sm">
                    <thead>
                      <tr>
                        <th className="text-right text-xs font-medium text-slate-400 dark:text-slate-500 pb-2">תיאור</th>
                        <th className="text-center text-xs font-medium text-slate-400 dark:text-slate-500 pb-2 w-12">כמות</th>
                        <th className="text-left text-xs font-medium text-slate-400 dark:text-slate-500 pb-2 w-24">מחיר</th>
                        <th className="text-left text-xs font-medium text-slate-400 dark:text-slate-500 pb-2 w-24">סה״כ</th>
                      </tr>
                    </thead>
                    <tbody>
                      {items.filter((i) => i.description.trim()).map((item, idx) => (
                        <tr key={idx}>
                          ...
                          <td className="py-1 text-center text-slate-500 dark:text-slate-400 tabular-nums">{item.quantity}</td>
                          <td className="py-1 text-left text-slate-500 dark:text-slate-400 tabular-nums">{fmt(Number(item.unit_price))}</td>
                          <td className="py-1 text-left font-medium text-slate-700 dark:text-slate-300 tabular-nums">
                ...
                <div className="px-5 py-4 space-y-1.5">
                  <div className="flex justify-between text-sm">
                    <span className="tabular-nums text-slate-500 dark:text-slate-400">{fmt(previewTotals.subtotal)}</span>
                    <span className="text-slate-600 dark:text-slate-400">לפני מע״מ</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="tabular-nums text-slate-500 dark:text-slate-400">{fmt(previewTotals.vat_amount)}</span>
                    <span className="text-slate-600 dark:text-slate-400">מע״מ (18%)</span>
                  </div>
                  <div className="flex justify-between font-bold text-base border-t border-slate-200 dark:border-slate-700 pt-2 mt-1">
                    <span className="tabular-nums text-slate-900 dark:text-slate-50">{fmt(previewTotals.total)}</span>
                    <span className="text-slate-800 dark:text-slate-200">סה״כ לתשלום</span>
                  </div>
          ...
          {step === 4 && (
            <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
              ...
              <div>
                <p className="text-lg font-semibold text-slate-900 dark:text-slate-50">מוכן להנפקה</p>
