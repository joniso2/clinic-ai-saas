...
export function AppointmentReceiptPrompt({ appointmentLabel, onIssue, onDismiss }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div
        className="modal-enter w-full max-w-[420px] bg-white dark:bg-slate-950 rounded-2xl shadow-2xl p-6 flex flex-col items-center gap-5 text-center"
        dir="rtl"
      >
        ...
        <div>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">תור נסגר בהצלחה</h2>
          {appointmentLabel && (
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{appointmentLabel}</p>
          )}
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">להפיק קבלה עכשיו?</p>
        </div>
